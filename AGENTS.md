...
| 文档 | 内容 |
|---|---|
| `pipeline/doc/tasks.md` `pipeline/doc/top_design.md` `pipeline/doc/isa.md` `pipeline/doc/modules/*.md` | 计组（实验一）：RV32I 5 级流水线 core（26 条冻结集、冲突策略、模块契约） |
| `UART/doc/tasks.md` `UART/doc/top_design.md` `UART/doc/interface.md` `soc/doc/firmware.md` `modules/*.md` | 汇编与接口（实验二）：UART SoC（统一编址 MMIO、.vh 固化、下板） |
| `SUBMISSION.md` | 两课提交物核对清单与当前总态 |
| `.coderabbit.yaml` | CodeRabbit 自动 review 配置 |

- 代码布局：计组 `pipeline/src/{rtl,defines,test,scripts}`；汇编 `UART/src/{rtl,test,scripts}`；SoC `soc/{rtl,test,xdc}`；文档 `pipeline/doc/`、`UART/doc/`、`soc/doc/`；参考工程 `ref/CPU/`（只读）。
...
- 跑脚本**直接执行看流式输出**；禁止 `| Select-Object -Last N` 之类截流。要看结果就 read/grep 打开的日志（`pipeline/src/scripts/out/*.log`）。
- 编码：全仓 UTF-8；含中文的 `.ps1` 需 UTF-8 BOM（PS 5.1 解析）；校验/修复 `pipeline/src/scripts/fix_encoding.ps1`（默认只报告）。

## 2. 测试（21 例）

- 运行器 `pipeline/src/scripts/run_tb.ps1`：**并行 worker，默认自动满核**（`min(#TB, 逻辑核数)`）。
  ```powershell
  powershell -File pipeline/src/scripts/run_tb.ps1                    # 全量 ~20-30s
  powershell -File pipeline/src/scripts/run_tb.ps1 -Case prog_demo     # 按关键字子集
  powershell -File pipeline/src/scripts/run_tb.ps1 -Jobs 8             # 限制并发；-Jobs 1=串行
  powershell -File pipeline/src/scripts/run_tb.ps1 -TimeoutSec 60      # 每例预算（须 ≥1）
  ```
...
- 性能：`pipeline/src/scripts/run_perf.ps1`（5 档，汇总 out/perf_summary.csv）。

## 3. GitHub 工作流（CodeRabbit review；**远程操作须用户显式授权**）

> ⚠️ **硬约束（最高优先级）**：未经用户显式要求，**禁止执行任何远程操作**——`git push`、`gh pr create`、`gh pr merge`（含 `--delete-branch`）一律不做，也不得用其它方式改写远端。
> 默认行为：只在**本地**建分支/提交，完成后报告改动与待办，等待用户指令；只有用户明确说"推送 / 开 PR / 合并"时，才执行对应动作（一次授权只对应一次明确指令）。

**默认分支 = `dev`，所有 PR 指向 `dev`。** 当用户要求走 PR 流程时：

1. 基于最新 dev 建分支（语义前缀：`feature/`、`fix/`、`opt/`、`docs/`、`ci/`）：
   `git checkout -b <前缀>/<主题> origin/dev`
2. 本地提交（`git commit …`）；**用户要求推送时**再 `git push -u origin <分支>`
3. **用户要求开 PR 时**：`gh pr create --base dev --head <分支> --title … --body …`
4. **CodeRabbit 自动审查**（PR 一旦存在；`.coderabbit.yaml`：非 draft、base dev、每次 push 增量、不自动暂停）。等待/检查：
...
5. 采纳意见 → 修代码 → **本地定向验证** → 本地 commit；**用户要求推送时** push → 循环直到 check `pass`。采纳后 CodeRabbit 会给评论标 `✅ Addressed in commit …`。
6. **用户要求合并时**：
   ```powershell
...
**经验/边界：**
- **远程授权边界（重要）**：push / 开 PR / 合并 PR 必须由用户显式要求；agent 可自行在本地建分支与提交，但**不得推送**；用户未要求时不要"顺手"执行远程操作。
...
- **僵尸 Vivado 进程**会让一切变慢/看似卡死（UART soc_top 综合曾异常退出仍空转占核）。遇疑似卡住：先 `Get-Process` 或 `Get-CimInstance Win32_Process`（看命令行归属）再清理，勿直接全杀。
...
| 用途 | 命令 |
|---|---|
| 全量回归（并行） | `powershell -File pipeline/src/scripts/run_tb.ps1` |
| 定向回归 | `powershell -File pipeline/src/scripts/run_tb.ps1 -Case <kw>` |
| 汇编 → hex | `uv run --no-project --python 3.13.13 pipeline/src/scripts/simple_asm.py <asm>` |
