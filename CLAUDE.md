...
1. **先读文档再动代码**：涉及功能/结构先看 `AGENTS.md` §0 列出的文档（`pipeline/doc/*`、`UART/doc/*`、`soc/doc/*`、`SUBMISSION.md`）；修改设计语义先改文档单源。
2. **分支一律基于 `dev`，PR 指向 `dev`**；review 由 CodeRabbit 自动做（`.coderabbit.yaml`）。
3. **测试**：`powershell -File pipeline/src/scripts/run_tb.ps1`（并行、默认满核、全量 ~20-30s）。迭代可用 `-Case <kw>` 定向；**最终提交前跑一次全量**。
4. **汇编**：本机无 GNU riscv 工具链，用
   `uv run --no-project --python 3.13.13 pipeline/src/scripts/simple_asm.py <asm>`
