...
| 源代码 | `src/rtl/*.v`（pipeline_top 等 14 模块）、`src/defines/*.v`、`src/scripts/`（run_tb/run_perf/build_asm/fix_encoding/synth_check/create_vivado_proj） | ✅ | 综合自检通过；工程 `board/vivado/board.xpr` 本地生成（gitignore，不入库；入口 `exp1_vivado.bat`） |
...
| 源代码 | `exp2/src/rtl/*.v`（uart_tx/uart_rx/uart_ctrl/dbus_decode/reset_sync/soc_top）、`exp2/src/xdc/board.xdc`、`exp2/src/test/console.S + console_rom.hex + console_init.vh`、`exp2/src/scripts/`（run_tb/build_fw/synth_check/create_vivado_proj/board_runs/program_devices/uart_check） | ✅ | 8/8 回归 PASS；带固件综合自检通过（41 s）；工程 `board/vivado/exp2.xpr` 本地生成（gitignore；入口 `exp2/exp2_vivado.bat`） |
