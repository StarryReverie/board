...
| 源代码 | `src/rtl/*.v`（pipeline_top 等 14 模块）、`src/defines/*.v`、`src/scripts/`（run_tb/run_perf/build_asm/fix_encoding/synth_check/create_vivado_proj） | ✅ | 综合自检通过；工程 `board/vivado/board.xpr`（exp1 工程目录=仓库根 vivado/）本地生成（gitignore，不入库；入口 `exp1_vivado.bat`） |
