...
| **实验一独立上板（CPU core，不接 UART）** | `pipeline/doc/board_runbook.md`（综合/烧录/验收/拍摄/故障定位全套 + **§5.1 自检原理与故障矩阵 / §10 实测验收记录 / §10.4 加固后重建 bit**） | ✅ **构建 A 已上板验收通过（2026-09-14）**；🟡 **构建 C（自检加固 + ALU 级故障矩阵 5/5）待实板复测（约 3 分钟，步骤见 §10.4）** | 构建 A 判据全中：数码管 **`0000000F`**、**LED1–LED6 全亮 / LED7 灭**、按住 `RESET` 时只剩 LED0 心跳且松手立刻恢复（复位可重复）。构建 A bit SHA256 `4C3FAC64…EB97`（post-route **11,338 LUT / 3,808 FF / 0 BRAM / 34 IOB**，WNS −1.239 ns）；**构建 C**（PR #11 两轮 review 整改：自检由 2 个里程碑 → **逐项 22 项核对 + 独立累积机制 + 多机制签名**；**指令级变异 23/23 检出、ALU 级单点故障 5/5 检出**）bit SHA256 `EB8A5870…9F77`，post-route **12,165 LUT / 3,838 FF / 0 BRAM / 34 IOB**，WNS −1.430 ns。**时序如实记录：两次构建的 100 MHz 约束均未收敛（268 / 240 个违例端点），实测最差 200 条路径终点全部在 `u_cpu/`（核心组合读存储链），板级包装零贡献**，但**实测未妨碍功能运行**。另记板级坑：按键丝印与 FPGA 引脚不一致（丝印 `RESET(P9)` 实际接 **P15**；`PROG` 才是 **P9/PROGRAM_B**，误按会擦配置） |
| 汇报 PPT×2（中期、验收） | 内容骨架待建（素材同上 + perf 表 A–D） | ⬜ | 汇报人/署名待提供 |
| ≤5min 下板演示视频 | 实验一上板视频：拍摄脚本见 `pipeline/doc/board_runbook.md` §6（7 个镜头）；实验二 SoC 视频单独拍、单独归档 | ⬜ **待拍** | **实验一已下板成功 ⇒ 需补拍**（原"可豁免"的前提已不成立）。**拍摄前先烧 §10.4 的构建 C bit（自检加固固件）并完成复测**，这样视频里的 `0000000F` 才等于"22 项核对全对、且单 opcode 故障不会假 PASS"。两段视频分别写入对应报告；实验一视频不需要串口终端 |
...
- 2026-09-13：新增计组关键波形 4 项（五级总览/前递优先级/load-use 冻结/分支冲刷）与 §7 回归·性能汇总表，归档 `pipeline/doc/sim_shots/exp1/`；回归口径 21/21（新增 `fwd_priority` 独立场景不入回归）；表 D（Fmax/资源）本机综合空转，暂留综合侧。
