...
| in | `mmio_rdata` | 32 | uart_ip_top → rdata mux | 组合读回，未命中应稳定为 0 |
...
2. **写**：`sw` 在**访存段末沿**锁存（与 dmem 同步写同沿）——uart_ip_top 寄存器层的寄存器更新、TX 触发、RX_VALID 清除均以该沿为准；
...
- v1.3 2026-09-07：结构合并（信号与时序零改动）：原 uart_ctrl 寄存器层并入 `uart_ip_top`（单模块 UART IP，删 uart_ctrl.v；功能不变）。
- v1.2 2026-09-07：结构调整（信号与时序零改动）：从机侧统一改称 `uart_ip_top`（IP 顶层，内部寄存器层仍 uart_ctrl）；SoC 上移 `../soc/`；§1 注：soc_top 内把 `reg_off[1:0]` 适配为 IP 的 `addr[3:0]={reg_off,2'b00}`。
