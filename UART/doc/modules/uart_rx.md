...
- 上游：板级引脚（经 soc_top ← N5，内部先打两拍防亚稳态）；下游：`uart_ip_top` 寄存器层（rx_data/valid）。
...
- 帧错误处理：基础版丢弃不置位（无错误寄存器，报告说明）；溢出（RX_VALID 未清又收完一字节）由 uart_ip_top 寄存器层丢弃新字节。

## 连接
- ← soc_top（rx，两拍同步后）；→ uart_ip_top 寄存器层（valid/data）。
