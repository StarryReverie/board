...
| require 原文要点 | 对应任务 |
|---|---|
| 基础团队任务：完成一种接口控制器，测试后形成 IP 核 | U10–U12 + U30（uart_tx/uart_rx/寄存器层 + 单测 → IP 打包） |
...
| U12 | 寄存器层（分频/挂起/丢弃/位义）——原独立 `src/rtl/uart_ctrl.v`，2026-09-07 并入 `src/rtl/uart_ip_top.v` | U10,U11 | TX/STAT/RX 字槽、位义逻辑 + `src/test/tb_uart_ctrl.v` | 槽地址/位义与 interface.md 一致；忙写丢弃；读 RX 清 RX_VALID；未命中读 0/写无操作（TB 经 IP 顶层例化） |
| U15 | `src/rtl/uart_ip_top.v`（**UART 控制器 IP 顶层**，寄存器层并入）+ `src/test/tb_uart_ip_top.v` | U12 | 独立 IP Core（单模块封装）+ IP 级独立测试 | IP 自含槽译码（addr[3:2]）；对外仅从机总线+串行引脚、无 CPU 依赖；TB：帧级逐位/STAT 位义/忙写丢弃/溢出丢弃/全双工同现全 PASS |
| U13 | `../soc/rtl/dbus_decode.v`（交付于 soc 集成层；例化于计组 pipeline_top MEM 段） | 计组 core M2 | 数据侧译码代码 | 规范单源=计组 `doc/modules/dbus_decode.md`；低区行为与 dmem 直连一致；窗口命中 TX/STAT/RX |
| U14 | `../soc/rtl/reset_sync.v` + `../soc/rtl/soc_top.v` | U12,U13,U15 + 计组 core | 整机装配、复位同步、引脚穿出 | soc_top=core+uart_ip_top+reset_sync 正确互联（addr={reg_off,2'b00} 适配）；rst_n(键)→异步置位/同步释放→rst(异步高有效)；无悬空；Vivado 综合通过 |
...
| 任务 | 内容 | 产出 | 验收标准 |
|---|---|---|---|
| U40 | 汇编固件（固定程序 console）：`.equ` 内存映射头、putc/getc、banner+回显主循环 | `../soc/test/console.S` → `.hex/.vh`（verify_hex.py 校验） | 上电自跑打印 banner；键盘回显往返正确；全指令在 26 条冻结集内；`.hex`/`.vh` 一致 |
| U41 | 机器码与文档：指令展开说明、内存映射说明 | `../soc/doc/machine_code.md` | 机器码可 objdump/脚本核对（方案 B：无自定义指令） |
...
| 9/8–9/12 | 寄存器层 + 从机 TB；固件与镜像（依赖计组 core M2 与 mmio 总线端口） | 成员 2/3 |
...
| 成员 2 | 寄存器层（寄存器/位义/忙丢弃/读清位，并入 `uart_ip_top`）+ `uart_ip_top`（IP 顶层与 IP 级独立测试）+ `dbus_decode` 代码交付与从机口适配；从机单测 | `src/rtl/uart_ip_top.v`（含寄存器层）、`src/test/tb_uart_ctrl.v`、`src/test/tb_uart_ip_top.v`、`../soc/rtl/dbus_decode.v`、寄存器/位义说明 |
| 成员 3 | 汇编固件（putc/getc/banner/echo）、机器码生成与校验、系统 TB 的串行监视器与断言 | `../soc/test/*.S → .hex/.vh`、`../soc/test/tb_soc_*.v` 软件部分、机器码说明（../soc/doc/firmware.md） |
| 成员 4 | SoC 装配（soc_top/reset_sync）、XDC（T5/T4/N5/P15）、综合时序、下板与演示视频、提交物整合 | `../soc/rtl/soc_top.v`、`../soc/rtl/reset_sync.v`、`../soc/xdc/board.xdc`、下板记录、报告/PPT 格式统一 |
