...
- **DMEM**（数据）：字节地址 `0x0000_0000` 起，容量 `DMEM_WORDS`（默认 1024×32bit=4KB），可读写。IMEM/DMEM 为**相互独立的地址空间**（哈佛），同为 0 起互不冲突。容量宏可参数化：**下板 build 缩容 IMEM 512B/DMEM 256B**（组合读寄存器阵列容量约束，见 `src/defines/const_define.v` 头注；仿真与程序契约仍默认 4KB）。
