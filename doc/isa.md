...
- 版本：v1.3（2026-09-04：§4 补 MMIO 统一编址定稿映射（方案 B，不加 in/out）；指令清单仍 26 条）。参照 RISC-V RV32I 官方编码；宏沿用 `ref/CPU/defines/`。供汇编器、校验脚本、TB 与报告引用。
...
- HALT：无停机指令；程序末尾以自循环 `halt: beq x0,x0,halt` 收尾（TB 判 PC 连续不变即停机并比对终值）。
- **MMIO（统一编址，方案 B 定稿 v1.3）**：外设与内存共用同一地址空间与同一组访存指令——**`lw`=外设读（in/r）、`sw`=外设写（out/w）**，不新增专用 in/out 指令（指令清单仍 26 条）。本系统实现一个 UART 从机（映射定稿与 top_design §9.2 / dbus_decode.md 一致，字访问 4 对齐）：
  ...
  - `0x0000_4004` STAT 槽：`lw` 读 bit0=TX_BUSY（1=发送中）、bit1=RX_VALID（1=收到未读字节）；写无操作。
