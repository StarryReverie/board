# decode（主译码 + 立即数扩展，组合）模块文档

- 位置：`decode.v`（译码段纯组合）。
- 参考 `ref/CPU/instr_decoder.v`（拆字段）与 `ref/CPU/control.v`（译码）；改造：按 §2 冻结 26 条全译码、控制信号成组输出。
...
| out | src_a / src_b | 2/2 | ALU A/B 源选择（见 top_design §2） |
...
## 立即数（按 isa.md §2）
- I 型(除移位)：`imm = {{20{inst[31]}},inst[31:20]}`；移位 imm=`{27'b0,inst[24:20]}`（shamt）。
- S 型：`{{20{inst[31]}}, inst[31:25], inst[11:7]}`；B 型：`{{19{inst[31]}}, inst[31], inst[7], inst[30:25], inst[11:8], 1'b0}`。
- U 型(lui)：`imm = {inst[31:12], 12'b0}`；J 型(jal)：`{{11{inst[31]}}, inst[31], inst[19:12], inst[20], inst[30:21], 1'b0}`。

## 连接
- inst ← if_id.inst；输出 → regfile(rs1/rs2 地址)、id_ex 的 ctrl/imm/rd 字段、hazard_unit(rs1/rs2 作 load-use 判断)。
