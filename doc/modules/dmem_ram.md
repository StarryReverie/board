# dmem_ram（数据存储器 RAM）模块文档

- 位置：`dmem_ram.v`（访存段存储）。参考 `ref/CPU/ram.v`（wmask 字节写）；保留同步写、周期内组合读。
- 与 IMEM 物理分离 → **结构冒险化解**。
...
- 读（组合）：地址来自 EX/MEM 寄存器（周期内稳定），数据在访存周期内就绪 → 访存段末沿由 MEM/WB 捕获。
- 复位不清存储（数据由程序自初始化，同单周期）。

## 读写同拍同地址语义
- 同步写 + 组合读：同一地址在该拍先出旧值（读在写生效前）→ lw 必须晚于 sw 一拍（前递/旁路已保证依赖顺序），测试程序遵循。

## 连接
- 地址/wdata/we ← ex_mem；rdata → mem_wb。
