# pipeline_top（顶层装配）模块文档

- 位置：`pipeline_top.v`。例化并互联全部模块；端口对齐参考 `ref/CPU/cpu_top.v`（仅 clk/rst）。
...
dmem_ram (addr=exmem_alu_result, wdata=exmem_wdata, we=exmem_mem_write) → rdata
...
```
- 控制信号打包约定：各 ctrl 位按需以离散 wire 互联（与 top_design §2 字段一致）；或按 1 条 ctrl 总线打包后逐位拆，编码时在 top_design 附录登记打包位序。
- HALT 观测：测试经层次引用读 regfile/dmem/idex_pc；溢出 `of` 可经层次引用观察。

## IMEM 装载
- `imem_rom` 内含 `include "imem_init.vh"`（综合用字面量装载）与 TB `$readmemh`（仿真）；两路内容由 `verify_hex.py` 校验一致。
