...
- 位置：`regfile.v`（ID 段读口 + WB 写口）。
- 参考 `ref/CPU/regfile.v`；改造：新增 **读旁路（write-first）**，消除 WB/ID 同拍写读陈旧。
...
## 功能/时序
- `reg [31:0] x[1:31]`，x0 恒 0；写 waddr=0 无效；waddr=0 读恒 0。
- 写：posedge clk（rst 清零 / we 且 waddr≠0 → x[waddr]<=wdata）。
- 读（组合 + 旁路）：
  - `rdata1 = (raddr1==0) ? 0 : (we && waddr==raddr1 && raddr1!=0) ? wdata : x[raddr1]`
  - rdata2 同理。

## 时序说明（与流水线各段）
- 读发生在 ID 段，写发生在 WB 段末沿；当"写回指令恰在 WB、消费者恰在 ID"同拍时，读口旁路直接返回当拍 wdata，避免陈旧值进入 ID/EX（其余 RAW 由前递覆盖，见 hazard_unit）。
