...
> 参考工程：`ref/CPU/`（大三课程设计，RV32I，独立 git，**保持原样不改**）
> 用途：流水线 CPU 的设计思想参考 + 后续报告"单周期"章节 + 性能对比数据源。

---

## 1. 工程结构速览（已通读核验 2026-09-02）
...
ref/CPU/
 ...
 ├─ alu.v              33 位进位 result，flags{CF,ZF,SF,PF,OF}（OF 溢出判定在内）
 ├─ ram.v              数据存储：reg[7:0] mem[0:4095]，wmask 字节写，组合读/边缘写
 ├─ defines/  instr_define.v（OP_*/F3_*/F7_*）、const_define.v（ALU_*/flags/IMM_*）
 ├─ utils/rars1_6.jar  RARS(RISC-V 汇编/仿真)，作汇编与结果参照
 └─ test/  test0.asm · test1.asm · test_sort.asm + 对应 rom.hex + makefile + 结果
```

## 2. 继承项（流水线直接沿用其思想/宏/模块）
...
| ALU | `alu.v` | 33 位进位、flags；`OF` 溢出判定满足"附加功能-溢出判断"；**计划近乎原样复用** |
| 寄存器堆 | `regfile.v` | x[31:1]、x0=0、waddr=0 不写 |
| 分支目标 | `cpu_top.v` L70 | 专用加法器 `branch_target=pc+imm_b_sext`（注释：需单独加法器） |
| 立即数拆法 | `instr_decoder.v` | imm_I/S/B/U/J 原始字段的位序重组（B/J 含位序打乱），沿用 |
| imm 扩展思路 | `cpu_top.v` | `imm_sel`+有/无符号 多路选扩展（流水线改在 ID 产 imm 直送 ID/EX） |
...
| 回归样例 | `test/test0·test1·test_sort.asm` | ORI/ADD/SUB/SW/LW/BEQ + 注释预期结果 → 迁移为流水线 T31 回归 |
| 存储 | `rom.v/ram.v` | 按字节数组 + wmask（sw 全字、预留字节写），小端重组 |

## 3. 必须改造项（单周期→流水线差异，记入 top_design）

| 差异 | 单周期做法 | 流水线做法 | 理由 |
|---|---|---|---|
| 控制信号 | control 组合直达各单元 | 随 4 组段间寄存器打拍，flush/bubble 清零 | 各段要独立判段信号 |
...
| 访存 | rom/ram **组合读** | IMEM/DMEM **同步读** | 组合读大数组不可综合为 BRAM/下板不稳 |
| IMEM 装载 | 仅 TB `$readmemh` | `.vh`(initial 字面量) 供综合 | 综合 ROM 初始化需要字面量 |
| 立即数 | 顶层 imm 多路选 | ID 段产出 imm 后随 ID/EX 走 | 简化 EX |
| ori 扩展 | 零扩展(与规范不符) | 按规范**符号扩展** | 正确性；参考测试全为正小立即数，迁移不受影响 |

## 4. 参考数据的"不越界"原则
流水线是**新设计**；`ref/CPU/` 的代码/数据仅作参考与对比源。任何可能被误读为"流水线实现"的脚手架均需标注；性能对比引用 `ref/CPU/` 大三数据时须注明来源（TBD，实验一冻结后追加）。
