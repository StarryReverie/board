...
| 地址 | 方向语义 | 寄存器 | 位义 |
|---|---|---|---|
| `0x0000_0000`–`0x0000_0FFF` | lw/sw | dmem | 数据 RAM 4KB |
| `0x0000_4000` | **sw = out/w** | TX | 写低 8 位=待发字节；TX_BUSY=0 时触发发送，忙时写丢弃；`lw` 读=0 |
| `0x0000_4004` | **lw = in/r** | STAT | bit0=TX_BUSY（1=发送中）、bit1=RX_VALID（1=有未读字节）；写无操作 |
