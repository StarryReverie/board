...
### 3.3 实测资源与时序（2026-09-14，本机 Vivado 2019.2，`xc7a100tcsg324-1`）

| 阶段 | Slice LUT | FF | BRAM | IOB | WNS (ns) | 等价 Fmax |
|---|---|---|---|---|---|---|
| post-synthesis | 11,412（18.00%） | 3,816（3.01%） | 0 | 34 | −1.236 | ≈ 89.0 MHz |
| **post-route（实现+布线）** | **11,356（17.91%）** | **3,812（3.01%）** | **0** | **34** | **−1.239** | **≈ 89.0 MHz** |

`create_clock -period 10.000`（100 MHz）约束下 **时序未收敛**，实现状态为
`route_design Complete, Failed Timing!`，240 个违例端点、TNS ≈ −199 ns。

**这不阻塞烧录**（`write_bitstream` 可正常产出），但必须如实理解：

1. **板级包装对关键路径零贡献**：实测最差 30 条路径的终点**全部**落在核心内部
   `u_cpu/u_id_ex/*`（ID/EX 段寄存器），没有一条落在板级逻辑
   （`br_seen`/`halt_seen`/`q_*`/`sig_*`/`to_cnt`）上。
   —— 板级监视器已把 `dbg_*` 先打一拍再比较；未打拍时最差路径确实落在板级
   （`exmem_rd → br_seen/CE`，WNS −1.442），打拍后回落到核心自身的最差路径（−1.236）。
...
4. **若上板后结果不稳定（LED 抖动 / PASS 时有时无）**，按以下优先级处理：
   ...
   - ② 把 CPU 改成"整机时钟使能"（每 2 拍推进一次 ⇒ 等效 50 MHz）——需给 `pipeline_top`
     增加全局 CE，属核心 RTL 改动，须同步文档并重跑 23 项回归；
...
> 记录口径：报告/答辩中引用本节时必须写明"**post-route WNS −1.239 ns（Fmax≈89 MHz），
> 100 MHz 约束未收敛；违例全部位于核心组合读存储链，板级包装无贡献**"，
> 不得写成"100 MHz 时序满足"。
...
验收签名不是"无条件写 15"，而是程序**自检后**才写：

```asm
xor  x28, x18, 15      # 里程碑1：bne 循环累加和 == 15 ？
xor  x30, x7,  20      # 里程碑2：ALU 异或结果 == 20 ？
or   x31, x28, x30     # 0 = 两项全对
sltu x5,  x0, x31      # 有错 → 1
sub  x6,  x0, x5       # 无错 → 0；有错 → 0xFFFFFFFF
xor  x7,  x6, 15
and  x8,  x7, 15       # 无错 → 0x0F；有错 → 0
sw   x8,  0(x0)        # ★ 只有全对才写出 0x0F
```

任一里程碑错误 → 写入 0 → 板上不出现 PASS（若此时程序仍能进入 HALT，
则 PASS 不亮但 LED7 也不亮，属"跑完但结果错"；若程序卡住则超时 → LED7 亮）。
...
> **注意：程序约 1 µs 就跑完，所有状态灯几乎同时亮，不存在"依次点亮"**——只能拍"终态 + 复位重跑"。
...
**为什么 `0000000F` 足以判定全链条正确**：该值只在程序**自检通过后**才写（先异或校验
"`bne` 循环累加和 = 15"与"ALU 异或结果 = 20"，任一项不符则写 0，见 §5.1）。
