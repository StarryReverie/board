...
| **B（2026-09-15）** | 118 条指令 / 468 B（自检加固，见 §10.4） | 12,311（19.42%）/ 3,854（3.04%） | **12,088（19.07%）/ 3,833（3.02%）** | 0 / 34 | **−1.468 ns** | ≈ 87.2 MHz |

`create_clock -period 10.000`（100 MHz）约束下两次构建**都没有收敛**，实现状态均为
`route_design Complete, Failed Timing!`：B 构建 263 个违例端点、TNS ≈ −268 ns
（A 构建为 240 个、TNS ≈ −199 ns）。B 相对 A 的 LUT +750 / FF +25 属预期方向
（imem 以 LUT 初值实现，固件变长后零区减少、可被常量折叠的逻辑变少），
但**未做"同 RTL 换固件"的对照综合，故不作为定量结论**。

**这不阻塞烧录**（`write_bitstream` 可正常产出，见 §10.4），但必须如实理解：

1. **违例全部落在核心内部，板级包装零贡献**：B 构建用
   `report_timing -delay_type max -max_paths 200` 实测，最差 200 条路径**全部**以
   `u_cpu/…` 为终点（`u_id_ex` 125 条、`u_pc_reg` 50 条、`u_if_id` 25 条），
...
2. **瓶颈是核心既有的组合读存储链**：`PC → imem（512 B 组合读 LUTRAM）→ 译码 → 寄存器堆组合读 → ID/EX`；
   B 构建实测的最差路径为 `u_cpu/u_mem_wb/*` → `u_cpu/u_id_ex/*`（同一条组合链的下游段）。
...
> / −1.468 ns（构建 B，Fmax≈87.2 MHz），两次构建的 100 MHz 约束均未收敛；违例全部位于核心
...
验收签名不是"无条件写 15"，而是程序**逐项自检通过后**才写：

```asm
addi x31, x0, 0        # x31 = 失配累积器
# 对每一项核对结果重复下列三步：
addi x28, x0, <期望值>
xor  x29, xR, x28      # 该项失配 → 非 0
or   x31, x31, x29     # 累积：任一项错则 x31 ≠ 0
...
sltu x5,  x0, x31      # 有错 → 1
sub  x6,  x0, x5       # 无错 → 0；有错 → 0xFFFFFFFF
addi x27, x0, 15
xor  x7,  x6, x27
and  x8,  x7, x27      # 无错 → 0x0F；有错 → 0
sw   x8,  0(x0)        # ★ 只有全部核对项正确才写出 0x0F
```

**核对范围（24 项）**：阶段1 的 7 项 ALU 结果（`add/sub/and/or/xor/slt/sltu`）、
`lui`+`srli` 的 4 项折半核对（x10/x11 用左右移折成小常数，规避 `addi` 12 位立即数
放不下的问题）、阶段2 的 4 项访存与 load-use 前递结果（x13/x14/x15/x16）、
阶段3 的 2 项循环结果（x17/x18）、阶段4 的 7 项移位与有/无符号比较
（x19/x20/x21/x23/x24/x25/x26）。

**灵敏度已逐条验证（变异测试）**：`build/tb_mut.v`（build 侧临时验证，不入库）把
22 条覆盖指令逐条改成 NOP（`sltu` 一条改为交换操作数）后重跑程序，
**22/22 全部使签名 ≠ 0x0F、0 漏检**——即"某项功能坏了但板上仍写出 0x0F"在数学上
不成立。其中含两个边界情形：循环计数不递减（死循环）→ 根本不写签名；
签名掩码指令被改 → 写出非 0 的错值。

任一核对项错误 → 写入 0 或非 0x0F → 板上不出现 PASS（若此时程序仍能进入 HALT，
则 PASS 不亮且 LED7 也不亮，属"跑完但结果错"；若程序卡住则超时 → LED7 亮）。

> 边界说明：数值自检无法识别"故障结果恰好等于期望值"的情形（例如某条指令被删掉，
> 而它写的目的寄存器本来就该是 0）；这类情形由 `tb_prog_board_demo.v` 的结构化
> 断言与上面的变异测试覆盖，不属板上自检的能力范围。
...
> **注意：程序约 1.4 µs 就跑完（静态 118 条指令、动态 130 条 ≈ 140 拍 @100 MHz），所有状态灯几乎同时亮，不存在"依次点亮"**——只能拍"终态 + 复位重跑"。
...
| 5 | 1:40–2:05 | — | 近景轮流对准数码管与 LED | "`0x0F` 不是无条件写的：先自检『循环累加和 = 15』与『ALU 异或结果 = 20』，两项全对才写。所以这一位 `F` 等于证明前递 / load-use 停顿 / 分支冲刷 / 移位 / 自检链在实板全对" |
| 6 | 2:05–2:25 | 切屏幕 | `run_tb.ps1` 的 **23/23 ALL PASS** 日志；综合/实现资源与时序报告 | "上板前 23 项回归全绿；上板口径 512 B/256 B，11,338 LUT / 3,808 FF / 0 BRAM" |
...
**为什么 `0000000F` 足以判定全链条正确（构建 A）**：该值只在程序**自检通过后**才写
（构建 A 的固件只异或校验"`bne` 循环累加和 = 15"与"ALU 异或结果 = 20"两项，任一项不符则写 0）。
所以它同时证明：`IMEM_INIT_VH` 固化生效、CPU 正确执行了 `lui`/移位/`sw→lw` 往返、
**EX-MEM 前递**、**load-use 停顿**、`bne` 循环 + **分支冲刷**、`slt·sltu` 与自检链，
并且板级监视器正确判出"签名 == 0x0F"与"HALT 自循环"。

> ⚠️ 覆盖强度说明：构建 A 的自检只覆盖两个里程碑——`lw`、load-use 前递、`lui`、移位、
> 有符号比较出错时它仍可能写出 `0x0F`（PR #11 review 指出）。**2026-09-15 已把自检加固为
> 逐项核对 24 项并重建 bit（构建 B，见 §10.4）**，复测通过后"签名 ⇒ 全对"的强度按 §10.4 计。
...
`lui`、移位、有符号比较出错时，签名仍可能是 `0x0F`，于是 PASS 不能推出这些功能正确。
本轮把阶段5 改为**逐项累积 24 项核对**的失配标志（覆盖 ALU 全类、`lui`+`srli`、
`sw→lw` 往返与两处 load-use 前递、循环计数与累加和、三种移位、有/无符号比较；
原理见 §5.1），并做了**变异测试**：22 条覆盖指令逐条改坏（NOP / 交换操作数）后
**22/22 全部使签名 ≠ 0x0F、0 漏检**（`build/tb_mut.v` + `build/run_mut.bat`，build 侧不入库）。

| 项 | 值 |
|---|---|
| 固件 | `exp1_board_demo.asm` → `_rom.hex`（**118 条指令 / 468 B**）→ `exp1_board_demo_init.vh`（补零 512 B） |
| 固件指纹 | `exp1_board_demo_init.vh` SHA256 `D6FEE73CCB36B182147CED9D3D4844E6419961F5D526C86A7C53252BC9ED1C30` |
| bit | `pipeline/vivado/board_exp1.runs/impl_1/exp1_board_top.bit`（3,825,899 B，2026-09-15 12:22:52） |
| **SHA256** | `4EC61A422BBD8CA53BE6E0B606292BD81D2A1DB198506963446510FB969FD214` |
| 资源（post-route） | **12,088 LUT（19.07%）/ 3,833 FF（3.02%）/ 0 BRAM / 34 IOB**（post-synth 12,311 / 3,854） |
| 时序（post-route） | WNS **−1.468 ns**（≈87.2 MHz），263 个违例端点、TNS ≈ −268 ns；**违例全部在 `u_cpu/…`**（见 §3.3） |
...
| 构建 A 的 bit | 已备份为 `build/exp1_board_top_OLD_4C3FAC64.bit`（内容即 §10.1 的 SHA，可回退） |
...
> 复测的额外意义：加固后"板上出现 `0000000F`"⇒ **24 项核对全部正确**（含 `lw`、
> load-use 前递、`lui`、三种移位与有/无符号比较），比构建 A 的结论强得多；
> 报告里应同时给出"变异测试 22/22 检出"作为该判据的灵敏度证据。
