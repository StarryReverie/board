...
- 版本：v1.4（2026-09-06：§4 STAT.TX_BUSY 位义修正——bit0=1 表示发送忙（含挂起待发与移位中），对齐 uart_ctrl（现并入 uart_ip_top）修复；指令清单仍 26 条）。参照 RISC-V RV32I 官方编码；宏沿用 `ref/CPU/defines/`。供汇编器、校验脚本、TB 与报告引用。
...
- **DMEM**（数据）：字节地址 `0x0000_0000` 起，容量 `DMEM_WORDS`（默认 1024×32bit=4KB），可读写。IMEM/DMEM 为**相互独立的地址空间**（哈佛），同为 0 起互不冲突。容量宏可参数化：**下板 build 缩容 IMEM 512B/DMEM 256B**（组合读寄存器阵列容量约束，见 `pipeline/src/defines/const_define.v` 头注；仿真与程序契约仍默认 4KB）。
...
- HALT：无停机指令；程序末尾以自循环 `halt: beq x0,x0,halt` 收尾。停机判据口径（v1.4 修订）：not-taken 预测下自循环的 PC 呈周期往复、不会"连续不变"，故 TB 采用**固定周期运行后采样比对**（寄存器堆/dmem 终值），见 `pipeline/src/test/tb_prog_*.v`。
...
- v1.4 2026-09-06：§4 MMIO STAT 槽 TX_BUSY 位义修正——bit0=1 表示发送忙（**含挂起待发与移位中**），对齐 uart_ctrl 实现（tx_accept 仅限完全空闲，挂起期写丢弃；该寄存器层现并入 uart_ip_top），消除连续 putc 丢字窗口；指令清单不变。
