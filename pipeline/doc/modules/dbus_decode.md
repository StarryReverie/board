...
- `dbus_decode.v`（MEM 段纯组合；代码在 `soc/rtl/`，SoC 集成层）。**实验二启用**（top_design §9.2、tasks.md §6 T40）；实验一核内 dmem 直连、不例化。职责：把一次 `lw/sw`（MEM 段）按地址路由到数据 RAM 或 MMIO 外设，并选出回写数据（rdata mux）。
...
| in | mmio_rdata | 32 | 命中外设读回（实验二接 uart_ip_top 读口） |
...
- 输入 ← ex_mem（alu_result→addr、mem_write→we）；dmem_rdata ← dmem；mmio_rdata ← uart_ip_top 读口。
- 输出 → rdata → mem_wb；`we & cs_dmem` → dmem.we；`we & cs_mmio` + reg_off → uart_ip_top 写口（soc_top 内适配 `addr={reg_off,2'b00}`）。

## 验收
- 低区行为与"dmem 直连"完全一致（实验一回归 H1–H5 不变）。
- 窗口按上表命中 TX/STAT/RX 槽：写 TX 触发发送（忙时丢弃语义在 uart_ip_top 侧）、STAT/RX 读回位义正确、RX 读后清 RX_VALID；未映射地址返回 0 且不产生写。

## 变更记录
- v1.1 2026-09-07：SoC 上移 `soc/`——代码位置注明 `soc/rtl/`；从机称谓 uart_ctrl→uart_ip_top（契约信号不变）。
- v1.0 2026-09-04：初版（定稿 v1.4 口径）。
