...
- 流水线是**新设计**；`ref/CPU/` 的代码/数据仅作参考与对比源，任何可能被误读为"流水线实现"的脚手架均需标注；性能对比引用大三数据时须注明来源（方案：`doc/perf_analysis.md`，T32–T35）。
- **编码结论（2026-09-06 全仓排查，`fix_encoding.ps1` 报告模式复核）**：`ref/CPU/` 及本仓全部文本均为**纯 UTF-8**，无 GBK 残留、无二次损坏。形如"鎸囦护閫氳矾"的乱码只在 **GBK/ANSI 解码环境**出现（Windows PowerShell 5.1 `Get-Content` 读无 BOM 文件、旧版记事本、GBK 终端）——是**显示假象，不是文件损坏**；**严禁对 ref 源再次转码/批量替换**。查看建议：`Get-Content -Encoding UTF8`、VS Code/现代记事本。仓库 `.ps1` 统一 **UTF-8 BOM**（PS 5.1 解析含中文脚本必需，`run_tb.ps1`/`build_asm.ps1` 已符合）；校验工具 `src/scripts/fix_encoding.ps1`（默认仅报告，`-Apply` 才写回）。
