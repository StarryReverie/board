...
- 流水线是**新设计**；`ref/CPU/` 的代码/数据仅作参考与对比源，任何可能被误读为"流水线实现"的脚手架均需标注；性能对比**以组内成员大三阶段的单周期实验数据为基准**，引用时须逐项注明出处与测试条件（方案：`pipeline/doc/perf_analysis.md` v2.0 §4）。
...
- 2026-09-13：§4 性能对比口径改为"**以组内成员大三阶段单周期实验数据为基准**"（方案见 `doc/perf_analysis.md` v2.0 §4；不再复测 `ref/CPU` 副本）。
