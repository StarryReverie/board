...
#   用法：vivado -mode batch -source pipeline/src/scripts/create_vivado_proj.tcl
#   产出：pipeline/vivado/board.xpr（exp1 工程目录 = pipeline/vivado；
#         本脚本只清理其中 board.* 自身产物，与 UART/vivado 互不影响）
#   入口：pipeline/exp1_vivado.bat（双击打开/自动重建）
#   工程口径：
#     - part: xc7a100tcsg324-1（EES-338）
#     - sources_1（设计源）: pipeline/src/rtl/*.v，top=pipeline_top
#     - sim_1（仿真源）    : pipeline/src/test/tb_*.v，top=tb_pipeline_top（默认）
#     - include_dirs = pipeline/src/（模块内 `include "defines/*.v" 由此解析）
#     - 文件均为"原位引用"（不拷贝进工程，代码仍以 pipeline/src/ 为准）
#   注：程序级 TB（tb_prog_*）经 $readmemh 读 .hex，GUI 直跑时需把
#       pipeline/src/test/*.hex 复制到 xsim 工作目录；推荐用 run_tb.ps1 跑仿真。
#=====================================================================

set scr  [file dirname [file normalize [info script]]]
# 路径推导：scr=pipeline/src/scripts, src=pipeline/src, root=pipeline,
#           proj=pipeline/vivado/board（exp1 工程目录 = pipeline/vivado）
set src  [file dirname $scr]
set root [file dirname $src]
set proj [file join $root vivado board]

# 清理旧工程（idempotent；只清本 vivado 下 board.*，与 UART/vivado 互不影响）
if {[file exists $proj.xpr]} { close_project -quiet }
foreach suf {.xpr .cache .hw .ip_user_files .runs .sim} {
    file delete -force "$proj$suf"
}

# 建工程（pipeline/vivado/ 内生成 board.xpr 及产物）
