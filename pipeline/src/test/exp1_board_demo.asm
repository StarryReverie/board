...
#   自检：阶段5 把**每一项**声明的中间结果与期望值比较（`xor`/`sub` 交替），
#         经 `sltu` 归一后 **`add` 累加**进失配计数 x31（**不得改回 `or` 累加**：
#         `ALU_OR` 故障会把失配全部吞掉造成假 PASS，见阶段5 注释与
#         pipeline/doc/board_runbook.md §5.1）；只有 x31 == 0（全部相符）才把
#         0x0F 写入 dmem[0]，任一项错误则写入 0 → 板上不出现 PASS。
