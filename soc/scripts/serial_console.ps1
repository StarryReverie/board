...
#         它的关键性质：**只打印从串口读回的字节**，键盘那一路只往串口写、绝不本地显示
...
#   行为：
#     * 默认：只打印 FPGA 发来的内容（无本地回显）；收到的内容同时追加写入日志文件
#     * -LocalEcho：额外把"真正捕获并发出的键"显示成 [TX:<字符>]（**排错用；录制时必须关**，
#       否则屏幕上就分不清"是 FPGA 回显还是本地画的"）
#     * 键入即发送（Enter 发 CR、Backspace 发 BS），Ctrl+C 退出（日志已 AutoFlush，不丢）
...
Write-Host ("[OK] type to send (Enter = CR). Ctrl+C to quit.  LocalEcho={0}" -f [bool]$LocalEcho)
...
try {
    while ($true) {
        ...
        if ([Console]::KeyAvailable) {
            ...
            switch ($k.Key) {
                'Enter'     { $sp.Write("`r"); if ($LocalEcho) { Write-Host -NoNewline "[TX:CR]" } else { Write-Host -NoNewline "`r`n" } }
