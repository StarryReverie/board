#=============================================================================
# board.xdc — EES-338（依元素口袋计算机；**实测器件 `xc7a100tcsg324-1`**，用户手册误标 35T）板级约束
#   依据：E:\Homework\26-27-1\EES-338_UserManual_v1.0.pdf（2026-09-06 核验；器件型号以 JTAG idcode 实测为准）
#     §4  系统时钟：100 MHz 晶振 → FPGA 全局时钟脚 T5（SYS_CLK）
#     §6.1 复位键：FPGA_RESET → P15（S8/S6=RST、S7/S5=PROG；专用按键，
#          手册未明示电平极性——本设计按"低有效、按下复位"并加上拉；
#          **2026-09-14 实验一在同一块板上实测确认：P15 低有效、松键=运行**，
#          故无需反相（若换板实测相反，把 soc_top 的 rst_n 反相接入即可）。
#          实测坑：丝印 `RESET(P9)` 的按键实际接 **P15**；**`PROG` 才是 P9=`PROGRAM_B`，按下会擦配置**。
...
#   构建：建工程 `soc/scripts/create_soc_proj.tcl`（一条命令；综合自检 `UART/src/scripts/synth_check.tcl`），
#         完整口径见 soc/doc/board_runbook.md §1
#=============================================================================
